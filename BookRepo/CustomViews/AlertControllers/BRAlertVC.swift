...
class BRAlertVC: UIViewController {
    let containerView = BRContainerView()
    let titleLabel = BRTitleLabel(textAlignment: .center, fontSize: 20)
    let messageLabel = BRBodyLabel(textAlignment: .center)
    let actionButton = BRButton(colour: .systemMint, title: "Ok", systemImageName: "checkmark.circle")
